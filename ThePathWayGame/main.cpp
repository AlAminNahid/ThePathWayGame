...
namespace levelTwo{
    ...
    */
    ...
    void timer(int value) {
        ...
        if (noCollision(plane, obstacles) && noCollision(plane,transformedRotationPolygons) && noCollision(plane,transformedTranslationPolygons)){
            if (yMax <= 60.0f){
                yMin += step;
                yMax += step;
            }
        ...
        if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
            ...
            if (isInsideRect(normalizedX, normalizedY, x1Min, x1Max, y1Min, y1Max)) {
                ...
                if (currentScreen == 3){
                    ...
                    glMatrixMode(GL_PROJECTION);
                    glLoadIdentity();
                    glOrtho(-15.0f, 15.0f, yMin, yMax, -1.0f, 1.0f); // Match the polygon's range
                    glMatrixMode(GL_MODELVIEW);
                    glLoadIdentity();
